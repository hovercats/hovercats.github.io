#### Create a tiny init in rc

**DISCLAIMER**: Always do your own research, and don't blindly trust some random blogpost on the internet.
I also made this blogpost so it might peak peoples interest in the topics in this post, including my own.  
...
`rc` is the shell used by [plan9](https://en.wikipedia.org/wiki/Plan_9_from_Bell_Labs).  
`rc` has since been ported to various OS's, like Linux, and BSD's (maybe others aswell? not sure).  
...
This might imply that `rc` *lacks* features, but this is a feature, and not a bug.

Lets also give a crude explenation of the boot process of a Linux system:
1. Bootloader starts, which loads up the kernel. (This is optional, as we can use the kernel as bootloader with [efistub](https://www.kernel.org/doc/html/latest/admin-guide/efi-stub.html))
...
If your init is not PID 1, something is wrong, and the init should exit with and error.
...
- [`kill`](https://pubs.opengroup.org/onlinepubs/9699919799/utilities/kill.html#top) `-s TERM -1`
  - send TERM signal to all PIDs, except PID 1 (we omit this with the `-1` flag)
...
- I use `halt` from [ubase](https://git.suckless.org/ubase/), made by [suckless](https://suckless.org/)
- `halt` in our case will **imediately** turn of your machine, so its important for disks to have been properly unmounted before we run this, so we avoid data loss
